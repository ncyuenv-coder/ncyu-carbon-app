...
def apply_morandi_theme():
    ...
    <style>
        ...
        div[data-testid="stElementContainer"]:has(.purple-bg-marker) + div[data-testid="stVerticalBlock"] {
            background-color: #ECE8F2 !important; padding: 30px !important; border-radius: 12px !important; margin-bottom: 25px !important; border: 1px solid #D6CFE1;
        }
    ...
    with tab1:
        ...
        if df_inv.empty: 
            ...
            with st.form("send_form"):
                batch_name = st.text_input("📝 請輸入本次發送批次名稱", value=f"{now_year_roc}年度溫室氣體盤查範疇之氣體鋼瓶購買調查")
                data_year_roc = st.text_input("📅 盤查所屬年度 (將帶入信件 {年度} 變數中)", value=f"{now_year_roc-1}年度")
                ...
                if is_test_mode and not test_email:
                    ...
                    for (dept, mgr, email), group in grouped:
                        ...
                        for _, row in group.iterrows():
                            ...
                            links_html += f'<div style="margin-bottom: 8px; background-color: #FDFBF7; padding: 10px; border-left: 5px solid #D4A373;"><b>{campus} - 門牌 {room}</b>：<a href="{link}" style="color: #3498DB; font-weight: bold;">點此進入系統</a></div>'
                            dict_records_to_append.append([batch_name, dept, mgr, campus, room, email, token, link, "發送成功", now_time, "待回覆", ""])
                            
                        mail_content = batch_body.replace("{系所}", dept).replace("{老師名稱}", display_mgr).replace("{年度}", data_year_roc).replace("{links}", links_html)
