        ...
        def wrapper(*args, **kwargs):
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    err_str = str(e).lower()
                    # 擴大捕捉範圍：包含 429 頻率限制、500系列伺服器異常、連線中斷、Timeout 等
                    if any(x in err_str for x in ["429", "quota", "rate limit", "500", "502", "503", "timed out", "connection", "broken pipe", "readtimeout"]):
                        if attempt < max_retries - 1:
                            try:
                                st.toast(f"⏳ 系統讀取資料時遇到短暫網路延遲，正在為您自動重試... (第 {attempt+1}/{max_retries-1} 次)", icon="🔄")
                            except: pass
...
# [新增防護] 將開表動作納入重試裝甲
@with_retry(max_retries=3, base_delay=2.0)
def open_sheet_with_retry(gc_client, sheet_id):
    return gc_client.open_by_key(sheet_id)

@with_retry(max_retries=3, base_delay=2.0)
def get_worksheet_with_retry(sh, title):
    return sh.worksheet(title)

# [修正] 補上 ttl=3500，提早於 Token 1小時過期前汰換，避免產生死連線
...
def get_worksheets():
    try:
        gc_client, drive_svc = init_google_fuel()
        # [修正] 改用具備重試機制的函式呼叫 Google API
        sh = open_sheet_with_retry(gc_client, SHEET_ID)
        
        try: ws_eq = get_worksheet_with_retry(sh, "設備清單") 
        except: ws_eq = sh.sheet1 
        
        try: ws_rec = get_worksheet_with_retry(sh, "油料填報紀錄")
        except:
            try: ws_rec = get_worksheet_with_retry(sh, "填報紀錄")
            except: ws_rec = sh.add_worksheet(title="油料填報紀錄", rows="1000", cols="13")
            
        return gc_client, drive_svc, ws_eq, ws_rec
    except Exception as e: 
        st.error(f"燃油資料庫初始化連線失敗，請稍後重試: {e}")
        st.stop()

# 只在伺服器啟動或快取過期時，才會真正向 Google API 發送這層請求
gc, drive_service, ws_equip, ws_record = get_worksheets()

# [修改重點 1]：透過 _ws_equip 參數強迫忽略雜湊，保證 100% 命中快取，API 呼叫次數歸零！
@st.cache_data(ttl=3600, show_spinner=False)
def load_fuel_data(_ws_equip, _ws_record):
    df_e = pd.DataFrame()
    df_r = pd.DataFrame()
    
    try:
        records = get_all_records_with_retry(_ws_equip)
    ...
    try:
        data = get_all_values_with_retry(_ws_record)
        # 初始化空表防呆
        if not data or len(data) == 0:
            headers = ["填報時間", "填報單位", "填報人", "填報人分機", "設備名稱備註", "校內財產編號", "原燃物料名稱", "油卡編號", "加油日期", "加油量", "與其他設備共用加油單", "備註", "佐證資料"]
            append_rows_with_retry(_ws_record, [headers])
            data = [headers]
...
# 明確傳入連線物件，切斷對全域變數的依賴
df_equip, df_records = load_fuel_data(ws_equip, ws_record)
...
# [圖片智慧快取機制]
# ==========================================
# [修改重點 2]：將 cache_data 換成 cache_resource，此機制才能完美快取圖片等複雜物件，停止不斷向 Drive 要檔案！
@st.cache_resource(ttl=86400, show_spinner=False, max_entries=100)
