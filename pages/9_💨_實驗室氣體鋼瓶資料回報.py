...
def apply_morandi_theme():
    ...
    <style>
        ...
        div.stRadio > div[role="radiogroup"] > label { background-color: #E3F2FD !important; padding: 12px 24px !important; border-radius: 8px !important; border: 2px solid #90CAF9 !important; cursor: pointer; }
        ...
        div.stButton > button.orange-btn, div[data-testid="stDownloadButton"] button { background-color: #E67E22 !important; border: 2px solid #D35400 !important; color: white !important; font-size: 18px !important; font-weight: bold !important; padding: 12px 20px !important; border-radius: 6px !important; }
...
def get_pdf_preview_images(file_obj):
    try:
        ...
        st.error("🚨 系統錯誤：尚未安裝 `PyMuPDF` 套件，無法將 PDF 轉換為圖片！")
        return []
...
def get_drive_file_bytes_cached(file_url):
    try:
        ...
        return file_stream.getvalue(), mime, meta.get('name', 'downloaded_file')
    except Exception as e:
...
def create_report_docx(base_data, pur_data, status):
    ...
    if title.runs: title.runs[0].font.size = Pt(24) 
    
    doc.add_heading('【實驗室基本資訊】', level=2)
    t_base = doc.add_table(rows=8, cols=2)
    t_base.style = 'Table Grid'
    t_base.cell(0, 0).text = "系所"; t_base.cell(0, 1).text = str(base_data['dept'])
    t_base.cell(1, 0).text = "老師"; t_base.cell(1, 1).text = str(base_data['mgr'])
    t_base.cell(2, 0).text = "校區"; t_base.cell(2, 1).text = str(base_data['campus'])
    t_base.cell(3, 0).text = "門牌"; t_base.cell(3, 1).text = str(base_data['room'])
    t_base.cell(4, 0).text = "電子郵件"; t_base.cell(4, 1).text = str(base_data['mail'])
    t_base.cell(5, 0).text = "分機"; t_base.cell(5, 1).text = str(base_data['ext'])
    ...
    if status == "無購買" or not pur_data:
        ...
        # 🚀 智能單/多筆展開彙整
        df_p = pd.DataFrame(pur_data)
        for gas_type, group in df_p.groupby('鋼瓶氣體種類'):
            n_items = len(group)
            if n_items == 1:
                p_table = doc.add_table(rows=5, cols=2)
                p_table.style = 'Table Grid'
                p_table.cell(0, 0).text = "盤查年度"; p_table.cell(0, 1).text = "2026年"
                p_table.cell(1, 0).text = "鋼瓶氣體種類"; p_table.cell(1, 1).text = gas_type
                p_table.cell(2, 0).text = "購買日期"; p_table.cell(2, 1).text = str(group.iloc[0]['購買日期'])
                p_table.cell(3, 0).text = "購買量(公斤)"; p_table.cell(3, 1).text = f"{group.iloc[0]['年度氣體鋼瓶購買量(公斤)']} 公斤"
                p_table.cell(4, 0).text = "填報日期"; p_table.cell(4, 1).text = datetime.datetime.now().strftime('%Y/%m/%d')
            else:
                p_table = doc.add_table(rows=3 + n_items * 2, cols=2)
                p_table.style = 'Table Grid'
                p_table.cell(0, 0).text = "盤查年度"; p_table.cell(0, 1).text = "2026年"
                p_table.cell(1, 0).text = "鋼瓶氣體種類"; p_table.cell(1, 1).text = gas_type
                for i, (_, row) in enumerate(group.iterrows()):
                    p_table.cell(2 + i*2, 0).text = f"購買日期-序號{i+1}"
                    p_table.cell(2 + i*2, 1).text = str(row['購買日期'])
                    p_table.cell(3 + i*2, 0).text = f"購買量(公斤)-序號{i+1}"
                    p_table.cell(3 + i*2, 1).text = f"{row['年度氣體鋼瓶購買量(公斤)']} 公斤"
                p_table.cell(2 + n_items*2, 0).text = "填報日期"; p_table.cell(2 + n_items*2, 1).text = datetime.datetime.now().strftime('%Y/%m/%d')
        ...
        for idx, p in enumerate(pur_data):
            ...
            if not file_obj and file_url:
                ...
                if f_bytes:
                    ...
                    file_obj.name = f_name
                    file_obj.type = f_mime
            ...
            if file_obj:
                if file_obj.type in ['image/jpeg', 'image/png']:
                    ...
                    if img_streams:
                        ...
                        doc.add_paragraph(f"📄 附檔佐證單據：{file_obj.name} (請確認是否安裝 PyMuPDF 或直接於系統後台檢視)").alignment = WD_ALIGN_PARAGRAPH.CENTER
            elif file_url:
                doc.add_paragraph(f"📎 此單據已上傳至雲端硬碟備查，連結：{file_url}").alignment = WD_ALIGN_PARAGRAPH.CENTER
            else:
...
def main():
    ...
    if is_readonly: 
        ...
        if "form_data" not in st.session_state:
            df_pur_all = pd.DataFrame(fetch_pur_records())
            teacher_pur = []
            if not df_pur_all.empty:
                mask_pur = (df_pur_all['系所'].astype(str) == st.session_state.dept) & (df_pur_all['實驗室老師'].astype(str) == st.session_state.manager) & (df_pur_all['氣體鋼瓶所在位置實驗室門牌'].astype(str) == st.session_state.room)
                teacher_pur = df_pur_all[mask_pur].to_dict('records')
            ...
            for p in teacher_pur:
                ...
                pur_reconstructed.append({
                    "鋼瓶氣體種類": p.get("鋼瓶氣體種類", ""),
                    "購買日期": p.get("購買日期", ""),
                    "年度氣體鋼瓶購買量(公斤)": p.get("年度氣體鋼瓶購買量(公斤)", 0),
                    "file_url": p.get("購買單據連結", "") 
                })
            ...
            st.session_state.form_data = {
                ...
                "status": "有購買" if pur_reconstructed else "無購買",
                "inv": st.session_state.inventory, "pur": pur_reconstructed
            }
    ...
    if st.session_state.step == "input" and (not is_readonly or is_admin):
        ...
        with c4: e_room = st.text_input("實驗室門牌位置", value=def_room, help="若鋼瓶放置多個位置，請一併填寫，如A32-101、A32-103、A32-105")
        ...
        st.info("💡 請確認盤查範疇之氣體鋼瓶現況，若氣體種類或常備數量有異動，請直接修改；若某種氣體已不再使用，請將數量改為 0。")
        ...
        st.markdown("##### ➕ 新增其他使用之盤查範疇氣體種類")
        default_add_inv = max(0, len(saved_inv) - len(st.session_state.inventory))
        add_inv = st.number_input("新增之盤查範疇氣體種類與常備數量", min_value=0, max_value=5, value=default_add_inv)
        ...
        st.markdown("請登錄調查年度之盤查範疇氣體購買量及上傳購買佐證單據。 **若本年度無購買請直接選擇「無購買」**。")
...
""", unsafe_allow_html=True)
                ...
                if file_obj:
                    if file_obj.type in ['image/jpeg', 'image/png']:
                        ...
                        if img_streams:
                            ...
                            st.markdown(f"📄 **佐證單據 PDF 檔案已暫存** (請確認是否安裝 PyMuPDF 或由系統管理員檢視)")
                elif file_url:
                    st.markdown(f"📎 **[點此前往雲端檢視已上傳之單據檔案]({file_url})**")
        ...
        if is_readonly:
            st.info("💡 此為已回報之唯讀資料，無法再次修改或送出。若需修正請聯繫環安中心管理員(#7137)。")
            word_buf = create_report_docx(d, d['pur'], d['status'])
            st.download_button("📥 下載我的申報存查表 (Word)", data=word_buf, file_name=f"氣體鋼瓶申報存查表_{d['dept']}_{d['mgr']}.docx", use_container_width=True)
        else:
