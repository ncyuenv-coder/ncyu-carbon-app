...
# 管理員可在主畫面上方切換模式
if is_admin:
    mode = st.radio("檢視模式", ["前台：冷媒填報與查詢", "後台：管理統計儀表板"], horizontal=True, label_visibility="collapsed")
    st.markdown("---")
        ...
        if df_records.empty:
            ...
            if st.button("💾 儲存變更", type="primary"):
                try:
                    ...
                    ws_target.clear()
                    # 準備新資料 (含標題，轉為 List)
                    updated_data = [valid_edit_cols] + edited_df.astype(str).values.tolist()
                    # 寫入
