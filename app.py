            ...
            if selected_dept in VIP_UNITS:
                ...
                if target_sub_cat:
                    ...
                    with st.form("batch_form", clear_on_submit=True):
                        ...
                        batch_inputs = {}
                        # 7:3 Layout (V98 Design)
                        for idx, row in filtered_equip.iterrows():
                            c_card, c_val = st.columns([7, 3]) 
                            with c_card:
                                header_color = MORANDI_COLORS.get(row.get('統計類別'), '#D5DBDB')
                                st.markdown(f"""
                                <div class="batch-card-v98">
                                    <div class="batch-header" style="background-color: {header_color};">
                                        <span class="batch-header-title">⛽ {row['設備名稱備註']}</span>
                                        <span class="batch-header-qty">數量: {row.get('設備數量','-')}</span>
                                    </div>
                                    <div class="batch-body">
                                        🏢 <strong>部門:</strong> {row.get('設備所屬單位/部門','-')}<br>
                                        👤 <strong>保管人:</strong> {row.get('保管人','-')}<br>
                                        ⛽ <strong>燃料:</strong> {row.get('原燃物料名稱')}<br>
                                        🔢 <strong>財產編號:</strong> {row.get('校內財產編號','-')}
                                    </div>
                                </div>
                                """, unsafe_allow_html=True)
                            with c_val:
...
# ------------------------------------------
            ...
            if st.button("開始篩選未申報單位"):
                ...
                if not unreported.empty:
                    ...
                    for idx, (unit, group) in enumerate(unreported.groupby('填報單位')):
                        ...
                        st.markdown(f"""
                        <div class="unreported-block" style="background-color: {bg_color};">
                            <div class="unreported-title">🏢 {unit} (未申報數: {len(group)})</div>
                        </div>
                        """, unsafe_allow_html=True)
    ...
    # === Tab C: 儀表板 (V100.0 完整邏輯) ===
    with admin_tabs[2]:
        st.subheader("📊 動態管理儀表板")
        if not df_year.empty:
            c_mode, c_filt = st.columns(2)
            stat_mode = c_mode.radio("統計模式", ["依設備統計", "依單位統計"], horizontal=True)
            
            if stat_mode == "依設備統計": filter_options = ["全部燃油設備"] + DEVICE_ORDER
            else: filter_options = ["全部單位"] + sorted([x for x in df_equip['填報單位'].unique() if x!='-' and x!='填報單位'])
            ...
            if not df_dash.empty:
                ...
                k1.markdown(f"""<div class="kpi-card kpi-gas"><div class="kpi-title">⛽ 汽油總量</div><div class="kpi-value">{gas_sum:,.1f}<span class="kpi-unit"> L</span></div></div>""", unsafe_allow_html=True)
                k2.markdown(f"""<div class="kpi-card kpi-diesel"><div class="kpi-title">🚛 柴油總量</div><div class="kpi-value">{diesel_sum:,.1f}<span class="kpi-unit"> L</span></div></div>""", unsafe_allow_html=True)
                k3.markdown(f"""<div class="kpi-card kpi-total"><div class="kpi-title">💧 總用油量</div><div class="kpi-value">{total_sum:,.1f}<span class="kpi-unit"> L</span></div></div>""", unsafe_allow_html=True)
                k4.markdown(f"""<div class="kpi-card kpi-co2"><div class="kpi-title">☁️ 碳排放量</div><div class="kpi-value">{total_co2:,.4f}<span class="kpi-unit"> tCO2e</span></div></div>""", unsafe_allow_html=True)
                st.markdown("---")
                ...
                if stat_mode == "依設備統計":
                    if selected_filter == "全部燃油設備":
                        fig_cnt = px.bar(df_asset.groupby(['統計類別', '油品大類'])['設備數量'].sum().reset_index(), x='統計類別', y='設備數量', color='油品大類', barmode='group', text_auto=True, color_discrete_map={'汽油':'#52BE80', '柴油':'#F4D03F', '其他':'#95A5A6'})
                    else:
                        sub = df_asset[df_asset['統計類別'] == selected_filter]
                        fig_cnt = px.bar(sub.groupby(['填報單位', '油品大類'])['設備數量'].sum().reset_index(), x='填報單位', y='設備數量', color='油品大類', barmode='group', text_auto=True, color_discrete_map={'汽油':'#52BE80', '柴油':'#F4D03F', '其他':'#95A5A6'})
                else: # 依單位
                    if selected_filter == "全部單位":
                        # 雙層 X 軸 (單位 + 油品)
                        fig_cnt = px.bar(df_asset.groupby(['填報單位', '油品大類', '統計類別'])['設備數量'].sum().reset_index(), x=['填報單位', '油品大類'], y='設備數量', color='統計類別', title="各單位設備數量 (依油品分組)", height=600)
                    else:
                        sub = df_asset[df_asset['填報單位'] == selected_filter]
                        fig_cnt = px.bar(sub.groupby(['統計類別', '油品大類'])['設備數量'].sum().reset_index(), x='統計類別', y='設備數量', color='油品大類', barmode='group', text_auto=True, color_discrete_map={'汽油':'#52BE80', '柴油':'#F4D03F', '其他':'#95A5A6'})
                st.plotly_chart(fig_cnt, use_container_width=True)

                # 3. Chart C: 用油量 (Fix KeyError & Add Facet)
                st.subheader("📈 單位用油量統計")
                v_fuel = st.radio("檢視油品", ["全部", "僅汽油", "僅柴油"], horizontal=True)
                df_use = df_dash.copy()
                if "汽油" in v_fuel: df_use = df_use[df_use['油品大類']=='汽油']
                elif "柴油" in v_fuel: df_use = df_use[df_use['油品大類']=='柴油']
                
                # 決定堆疊欄位
                grp_col = '統計類別' if stat_mode == "依設備統計" and selected_filter == "全部燃油設備" else ('填報單位' if selected_filter == "全部單位" else '統計類別')
                df_use[grp_col] = df_use[grp_col].astype(str)
                
                data_c = df_use.groupby(['月份', '油品大類', grp_col])['加油量'].sum().reset_index()
                
                # 如果選全部，使用 Facet 來分開汽柴油 (避免混在一起)
                if "全部" in v_fuel:
                     fig_c = px.bar(data_c, x='月份', y='加油量', color=grp_col, facet_col='油品大類', title=f"每月用油趨勢 ({grp_col}堆疊 - 分油品)", text_auto='.1f', color_discrete_sequence=px.colors.qualitative.Prism)
                else:
                     fig_c = px.bar(data_c, x='月份', y='加油量', color=grp_col, title=f"每月用油趨勢 ({grp_col}堆疊)", text_auto='.1f', color_discrete_sequence=px.colors.qualitative.Prism)

                fig_c.update_layout(xaxis=dict(tickmode='linear', tick0=1, dtick=1), font=dict(size=14))
                st.plotly_chart(fig_c, use_container_width=True)

                st.markdown("---")
                # 4. Charts D & E
                c_d, c_e = st.columns([3, 2])
                with c_d:
                    st.subheader("🌲 碳排熱點分析")
                    df_dash['CO2e'] = df_dash.apply(lambda r: r['加油量']*0.0022 if '汽油' in str(r['原燃物料名稱']) else r['加油量']*0.0027, axis=1)
                    if not df_dash.empty:
                ...
                with c_e:
                    st.subheader("🍩 油品結構分析")
                    sun_d = df_dash.groupby(['填報單位', '油品大類'])['加油量'].sum().reset_index()
                    if not sun_d.empty:
                        fig_sun = px.sunburst(sun_d, path=['油品大類', '填報單位'], values='加油量', color='油品大類', color_discrete_map={'汽油':'#52BE80', '柴油':'#F4D03F', '其他':'#95A5A6'})
                        fig_sun.update_traces(textfont=dict(size=16))
                        st.plotly_chart(fig_sun, use_container_width=True)

            else: st.warning("此篩選條件下無資料。")
        else: st.info("尚無該年度資料。")

    st.markdown('<div class="contact-footer">管理員系統版本 V100.0 (Century Flagship)</div>', unsafe_allow_html=True)